...
import numpy as np
import pandas as pd
def check_nac_para(k, normalize=False, upto=False, alphabet='ACGU'):
    ...
    try:
        if not isinstance(k, int) or k <= 0:
            ...
            raise ValueError("Error, parameter alphabet must be 'ACGU'.")
    except ValueError:
        raise
...
class Kmer():
    def __init__(self, k=1, normalize=False, upto=False, alphabet="ACGU"):
        """
        :param k: int, the k value of kmer, it should be larger than 0.
...
class IDkmer():
    def __init__(self, k=6, upto=True, alphabet='ACGU'):
        """
        :param k: int, the k value of kmer, it should be larger than 0.
        :param upto: bool, whether to generate 1-kmer, 2-kmer, ..., k-mer.
        :param alphabet: string.
...
vec = kmer.make_kmer_vec(open('S_data.txt'))
feature=np.array(vec)


data_new=np.matrix(feature)
data_kmer=pd.DataFrame(data=data_new)
data_kmer.to_csv('kmer_S.csv')
