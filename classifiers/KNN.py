...
for train, test in loo.split(X):
    cv_clf = KNeighborsClassifier(n_neighbors=50,weights="distance",algorithm="auto")
    X_train=X[train]
    y_train=y[train] 
    X_test=X[test]
    y_test=y[test]
...
yscore_sum.to_csv('y_score_KNN_S_Elastic_net.csv')
ytest_sum = pd.DataFrame(data=y_sparse)
ytest_sum.to_csv('y_test_KNN_S_Elastic_net.csv')
fpr, tpr, _ = roc_curve(y_sparse[:,0], y_score[:,0])
auc_score=result[6]
lw=2
...
data_csv.to_csv('KNN_S_Elastic_net.csv')