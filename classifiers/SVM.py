...
cv_clf = SVC(probability=True,kernel='rbf')
...
yscore_sum.to_csv('y_score_SVM_S_Elastic_net.csv')
ytest_sum = pd.DataFrame(data=y_sparse)
ytest_sum.to_csv('y_test_SVM_S_Elastic_net.csv')
fpr, tpr, _ = roc_curve(y_sparse[:,0], y_score[:,0])
auc_score=result[6]
lw=2
...
data_csv.to_csv('SVM_S_Elastic_net.csv')
