...
yscore_sum.to_csv('y_score_ERT_S_Elastic_net.csv')
ytest_sum = pd.DataFrame(data=y_sparse)
ytest_sum.to_csv('y_test_ERT_S_Elastic_net.csv')
fpr, tpr, _ = roc_curve(y_sparse[:,0], y_score[:,0])
auc_score=result[6]
lw=2
plt.plot(fpr, tpr, color='darkorange',
lw=lw, label='ERT ROC (area = %0.2f%%)' % auc_score)
...
data_csv.to_csv('ERT_S_Elastic_net.csv')
